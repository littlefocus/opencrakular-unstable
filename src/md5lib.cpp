...
void MD5::update (uint1 *input, uint4 input_length) {
	...
	if (finalized)  // so we can't update!
	{
		...
		panic("MD5::update:  Can't update a finalized digest!");
		return;
	}
...
void MD5::finalize (){
  ...
  if (finalized)
  {
	...
	panic("MD5::finalize:  Already finalized this digest!");
    return;
  }
...
unsigned char *MD5::raw_digest()
{
  ...
  if (!finalized)
  {
	...
	panic("MD5::raw_digest:  Can't get digest if you haven't \
...
std::string MD5::hex_digest()
{
	...
	char *s= new char[33];
	if (!finalized)
		panic("MD5::hex_digest:  Can't get digest if you haven't \
  			finalized the digest!");
...
// Although we coud use C++ style constants, defines are actually better,
